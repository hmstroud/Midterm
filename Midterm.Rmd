...
gs_loglik <- crossing(intercept = -0.09871,
                      slope = seq(0.28,0.48,.001),
                      resid_sd = 1.238) %>%
  rowwise() %>%
  mutate(logLik = lik_fun(slope, intercept, resid_sd)) %>%
  ungroup()
#filter for max loglik, look at slope
slope_prof <- gs_loglik %>%
  group_by(slope) %>%
  filter(logLik == max(logLik)) %>%
  ungroup()

slope_prof_plot <- ggplot(data = slope_prof) +
  geom_line(aes(x=slope, y=logLik))+
  ylab("Log Likelihood") + xlab("Slope") 
slope_prof_plot

#95 CI, CI= 1.92*SE 
#gs_loglik %>% filter(logLik > max(logLik) - 1.96) %>% filter(row_number()==1 | #row_number()==n())
